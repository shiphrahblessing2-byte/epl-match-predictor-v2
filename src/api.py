...
def predict_batch_endpoint(req: BatchRequest):
    ...
    try:
        results = predict_batch([m.model_dump() for m in req.matches], db)
...
def upcoming_predictions():
    """
    Return predictions for the next matchweek (EPL only).
...
def rolling_accuracy():
    """Return rolling accuracy over the last 10 predictions with known outcomes."""
    try:
        rows = []
        try:
            resp = (
